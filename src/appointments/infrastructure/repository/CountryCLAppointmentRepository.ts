...
export class CountryCLAppointmentRepository implements CountryAppointmentRepositoryInterface {
  private docClient: DynamoDBDocumentClient;
  private logger = createLogger('CountryCLAppointmentRepository');
  private tableName: string;

  constructor() {
    const dynamoClient = new DynamoDBClient({});
    this.docClient = DynamoDBDocumentClient.from(dynamoClient);
    this.tableName = process.env.CL_APPOINTMENTS_TABLE || '';
  }
